...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $phone = trim($_POST['phone'] ?? ''); $password = $_POST['password'] ?? ''; $confirm = $_POST['confirm_password'] ?? '';
    ...
    if (empty($errors)) {
        ...
        if (mysqli_stmt_num_rows($stmt) > 0) $errors[] = 'Email already exists.';
    }
    if (empty($errors)) {
        $hashed = password_hash($password, PASSWORD_DEFAULT); $role = 'user';
        $stmt = mysqli_prepare($conn, "INSERT INTO users (full_name, email, phone, password, role) VALUES (?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sssss", $full_name, $email, $phone, $hashed, $role); mysqli_stmt_execute($stmt);
        setFlash('success', 'Account created! Please sign in.'); header('Location: login.php'); exit;
    }
}
?>
...
<div class="auth-page"><div class="auth-card">
    ...
    <form method="POST">
        ...
        <div class="form-row">
            ...
            <div class="form-group"><label><i class="fas fa-check-circle"></i> Confirm</label><input type="password" name="confirm_password" class="form-control" placeholder="Repeat" required></div>
        </div>
